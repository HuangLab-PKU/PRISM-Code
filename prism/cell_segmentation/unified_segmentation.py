...
def _setup_tensorflow_gpu():
    """Ensure TensorFlow sees GPU and enable memory growth before any model load."""
    try:
        ...
        if gpus:
            try:
                ...
                print(f"TensorFlow: using GPU(s) {[g.name for g in gpus]}")
            except RuntimeError as e:
                print(f"TensorFlow GPU config warning: {e}")
        else:
            print("TensorFlow: no GPU found, using CPU. (Check CUDA/cuDNN and that CUDA_VISIBLE_DEVICES is not set to empty.)")
    except Exception as e:
        print(f"TensorFlow device check failed: {e}")
