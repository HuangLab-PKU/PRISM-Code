...
main_picture = Path('path_to_PRISM_code_src/gene_calling/main_page.png')
...
from gene_calling.data_preprocess import overview
...
# from gene_calling.preprocess import 
parameter_layout = [
...
while True:
    ...
    if active_tab == tab_titles[1]:
        ...
        if event == 'total_submit':
            if intensity_file is not None:
                from gene_calling.data_preprocess import gau_hist
                from gene_calling.data_preprocess import gaussian_blur
                gau_0 = values['gau_0']
                gau_1 = values['gau_1']
        ...
        if event == 'layer_submit':
            from gene_calling.data_preprocess import gau_hist_by_layer
    ...
    if active_tab == tab_titles[2]:
        ...
        if event == 'Perform GMM':
            from gene_calling.GMM_and_visualization import GMM_by_layer, GMM_visualization
            from gene_calling.quantitative_evaluation import conut_distribution
    ...
    if active_tab == tab_titles[3]:
        ...
        if event == '-DONE-':
            from gene_calling.mannual_thre import relabel
        ...
        if event == '-VISUALIZE-':
            if intensity_thre is not None:
                from gene_calling.GMM_and_visualization import visualization
                visualization(
    ...
    if active_tab == tab_titles[4]:
        current_tab_index = 4
        if event == '-SHOW_EV-':
            from gene_calling.quantitative_evaluation import calculate_cdf, conut_distribution, cdf_heatmap
